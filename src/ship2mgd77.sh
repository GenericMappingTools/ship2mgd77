...
if [ -s $procdir/${id}_rgrav_mgal ]; then
    ...
    if [ $sample2depthtime -eq 0 ]; then # Use grav_sample_interval
        awk '{printf "%04d:%03dT%02d:%02d:%02d.%03d %10.9f %10.9f \n",$1,$2,$3,$4,$5,$6,$9,$8}' $procdir/${id}_pos-mv_clean | gmt convert --FORMAT_CLOCK_IN=hh:mm:ss.xxx --FORMAT_DATE_IN=yyyy:jjj -fi0T -fo0t --FORMAT_FLOAT_OUT=%.12f | awk '{if ($1>prev && ($1>=st && $1<=en)) print prev=$1,$2,$3}' st=$STIME en=$ETIME | gmtconvert -fi0t -fo0T --FORMAT_DATE_OUT=yyyy:jjj --FORMAT_CLOCK_OUT=hh:mm:ss.xxx > $temp.txy
        ...
        gmtconvert $temp.tg.filt.samp -fi0t -fo0t | awk '{if ($1>prev && ($1>=st && $1<=en)) print prev=$1,$2}' st=$STIME en=$ETIME > $temp.tg.filt.samp2
